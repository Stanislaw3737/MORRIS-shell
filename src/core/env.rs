...
impl Env {
    ...
    /// Commit the current transaction, evaluating all expressions and applying changes atomically.
    /// This avoids double mutable borrow by splitting the steps.
    ...
    pub fn set_computed(&mut self, name: &str, value: Value, expr: &Expr) {
        ...
        if self.has_active_transaction() {
            ...
            if let Ok(transaction) = self.transaction_engine.get_active_transaction_mut() {
                ...
                // CRITICAL: Store in main environment so expressions are retrievable!
                self.variables.insert(name.to_string(), Variable::new(
        ...
        // Original behavior for non-transaction case (this part should work fine)
    ...
    pub fn set_direct(&mut self, name: &str, value: Value) {
        ...
        if self.has_active_transaction() {
            ...
            if let Ok(transaction) = self.transaction_engine.get_active_transaction_mut() {
                ...
                // Update local variable for transaction
                /*self.variables.insert(name.to_string(), Variable::new(
                    value.clone(),
                    false,
                    None,
