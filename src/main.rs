    ...
    if let Some(Target::Variable(var_name)) = &intent.target {
        ...
        if env.has_active_transaction() {
            ...
            if let Ok(num) = trimmed.parse::<i64>() {
                ...
                return Ok(format!("[🛠] Crafted: {} = {}", var_name, value.display()));
            }
            ...
            if trimmed.starts_with('"') && trimmed.ends_with('"') {
                ...
                env.set_direct(var_name, value.clone());
                return Ok(format!("[🛠] Crafted: {} = \"{}\"", var_name, inner));
            }
            ...
            match crate::core::expr::parse_expression(value_str) {
                Ok(expr) => {
                    ...
                    env.set_computed(var_name, placeholder, &expr);
                    
                    return Ok(format!("[🛠] Crafted: {} = {}", var_name, value_str));
                }
                Err(_) => {
        ...
        if value_str.contains('{') && value_str.contains('}') {
            match parse_interpolated_string(value_str, env) {
                Ok(interpolated) => {
                    ...
                    output.push_str(&format!("[+] {} = {}", var_name, interpolated));
            ...
            match parse_conditional_expression(value_str) {
                Ok(expr) => {
                    match crate::core::expr::evaluate(&expr, env) {
                        Ok(value) => {
                            env.set_computed(var_name, value.clone(), &expr);
                            
                            let mut output = String::new();
                            output.push_str(&format!("[+] {} = {} (conditional)", var_name, value.display()));
                            output.push_str(&format!("\n  Expression: {}", expr));
                        ...
                        Err(e) => {
                            ...
                            let mut output = String::new();
                            output.push_str(&format!("[?] {} = <?> (pending)", var_name));
                            output.push_str(&format!("\n  Expression: {}", expr));
                            output.push_str(&format!("\n  Note: {}", e));
            ...
            Ok(expr) => {
                match crate::core::expr::evaluate(&expr, env) {
                    Ok(value) => {
                        ...
                        env.set_computed(var_name, final_value.clone(), &expr);
                        
                        let propagated = crate::core::propagate::propagate_from(env, var_name)
                            .unwrap_or_default();
                        
                        let mut output = String::new();
                        output.push_str(&format!("[+] {} = {} (computed)", var_name, final_value.display()));
                        output.push_str(&format!("\n  Expression: {}", expr));
                    ...
                    Err(_) => {
                        ...
                        let mut output = String::new();
                        output.push_str(&format!("[+] {} = {} (direct)", var_name, value.display()));
...
/*fn execute_json_intent(
    ...
    if let Some(Target::Variable(name)) = &intent.target {
        let content = intent.parameters.get("content")
            .ok_or("JSON requires content")?;
        
        // Validate JSON
        match serde_json::from_str::<serde_json::Value>(content) {
            Ok(_) => {
                let json_value = crate::core::types::Value::Json(content.clone());
                env.set_direct(name, json_value.clone());
                Ok(format!("[+] Created JSON {}: {}", name, json_value.display()))
            },
            Err(e) => Err(format!("Invalid JSON: {}", e)),
        }
    } else {
        Err("JSON requires variable target".to_string())
    }
}*/
...
fn show_morris_logo(printer: &Printer) {
    ...
    println!(r"
        ...
        ║                                          ║
        ║    v1.0 • reactive propagation engine    ║
        ║                                          ║
        ╚══════════════════════════════════════════╝
    ");
